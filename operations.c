...
Pager *open_pager(char *filename)
{
    ...
    if (fd == -1)
    {
        printf("EXIT FAILURE -> open_pager failed");
        exit(EXIT_FAILURE);
    }
...
PageHeader *get_page(Pager *pager, PageID page_num)
{
    ...
    {
        printf("EXIT FAILURE -> get_page(): page_num out of range ( > MAX_TABLE_PAGES )");
        exit(EXIT_FAILURE);
    }
    ...
    if (pager->pages[page_num] == NULL)
    {
        ...
        if (page_num < num_pages_avaliable_on_disk) // go ahead and read bytes from disk into page
        {
            ...
            if (bytes == -1)
            {
                printf("EXIT FAILURE -> get_page(): read() failure on avaliable disk page");
                exit(EXIT_FAILURE);
            }
        }
        else
        {
            printf("EXIT FAILURE -> get_page(): NULL page not supported by disk");
            exit(EXIT_FAILURE);
        }
...
RowSlot get_row_slot(Pager *pager, uint32_t page_num, uint32_t row_num_offset)
{
    if (page_num > MAX_TABLE_PAGES)
    {
        printf("EXIT FAILURE -> get_row_slot(): page_num out of range ( > MAX_TABLE_PAGES )");
        exit(EXIT_FAILURE);
    }
    ...
    if (pager->pages[page_num] == NULL)
    {
        ...
        if (page_num < num_pages_avaliable_on_disk)
        {
            ...
            if (bytes == -1)
            {
                ...
                printf("EXIT FAILURE -> get_row_slot(): read() failure on avaliable disk page");
                exit(EXIT_FAILURE);
            }
        }
        else
...
void db_close(Table *table)
{
    ...
    if (result == -1)
    {
        printf("EXIT FAILURE -> db_close(): close() failure");
        exit(EXIT_FAILURE);
    }
...
void pager_flush(Pager *pager, uint32_t page_num, uint32_t size)
{
    ...
    if (offset == -1)
    {
        printf("EXIT FAILURE -> pager_flush(): lseek() Failure");
        exit(EXIT_FAILURE);
    }
